...
// ON RECEIVE MESSAGE
    ...
    constructor() {
        console.log('start the class constructor..')
    ...
    extractPosts = () => {
        const childrens = $('#m_group_stories_container .storyStream')
        if(childrens.children()){
            console.log('start extractPosts() ...', childrens.children().length )
            console.log('children', childrens.children());
        }

        childrens.children().each( (index, dom) => {

            const ft = JSON.parse(dom.dataset.ft)
            const item = $(dom);
            const item_id = ft.top_level_post_id;
            const sections = $(dom).children()
            const contents = $(dom).find("div.story_body_container>div").map((i,s)=>$(s).text()?$(s).text() : $(s).html() )
            const header = $(dom).find("div.story_body_container header strong").map((i,s)=>$(s).text()?$(s).text() : $(s).html() )
            const time = $(dom).find("div.story_body_container header abbr").first().text() ;

            console.log (index ,item_id,  ft.content_owner_id_new, header[0])
            //console.log (index, item_id, contents[0])
            ...
            owner.name = header[0];
            owner.id = ft.content_owner_id_new;
...
/*            sections.each(function (i, g) {
    ...
    handleScroll = () => {
        // console.log('start handleScroll() ...', (window.innerHeight), window.scrollY)
        if (window.innerHeight <= window.scrollY) {
            if ((window.innerHeight + window.scrollY) > this.lastScroll + window.innerHeight *2 ) {
                console.log('start handleScroll() indide ifs 2 ...', this.lastScroll)
                this.lastScroll = (window.innerHeight + window.scrollY);
                this.extractPosts();
                console.log('start handleScr');
